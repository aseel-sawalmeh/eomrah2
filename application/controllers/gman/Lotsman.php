...
class Lotsman extends Gman_Controller
{
    ...
    //autotranslate hotels
    public function hotels_batch_translate()
    {
        //bring hotels 
        $hotels = $this->lotsData->getHotels();
        // print_r($hotels[0]);
        // exit();
        foreach ($hotels as $hotel) {
            // $trhotelname = $this->trm->add_hotel_replace(
            //     ['S_ID' => $hotel->Hotel_ID, 'S_TYPE' => 'hotelname', 'en' => $hotel->Hotel_Name, 'ar' => trans($hotel->Hotel_Name, 'en', 'ar')]
            // );
            // $trhoteldesc = $this->trm->add_hotel_replace(
            //     ['S_ID' => $hotel->Hotel_ID, 'S_TYPE' => 'hoteldesc', 'en' => $hotel->Hotel_Description, 'ar' => trans($hotel->Hotel_Description, 'en', 'ar')]
            // );
            $trhoteladdress = $this->trm->add_hotel_replace(
                ['S_ID' => $hotel->Hotel_ID, 'S_TYPE' => 'hoteladdress', 'en' => $hotel->Hotel_Address, 'ar' => trans($hotel->Hotel_Address, 'en', 'ar')]
            );
            // if($trhotelname && $trhoteldesc){
            //     echo "HotelName: ".$hotel->Hotel_Name ." <br/> ". $hotel->Hotel_Description." Done <br />";
            // }else{
            //     echo "HotelName: " . $hotel->Hotel_Name . " <br/> " . $hotel->Hotel_Description . " Done <br />";
            //     break;
            // }
            if ($trhoteladdress) {
                echo "HotelName: " . $hotel->Hotel_Address . '/ ' . trans($hotel->Hotel_Address, 'en', 'ar') . " Done <br />";
            } else {
                echo "HotelName: " . $hotel->Hotel_Address . '/ ' .  trans($hotel->Hotel_Address, 'en', 'ar') . " Failed <br />";
                break;
            }
    ...
    public function roomTranslate()
    {
        // $rooms = $this->lotsData->getHotelRooms();
        $nonArRooms = $this->lotsData->nonArHotelRooms();
        foreach ($nonArRooms as $room) {
            $ar_name = trans($room->name, 'en', 'ar');
            $trRoom = $this->lotsData->roomsTranslate($room->roomId, $ar_name);
            if ($trRoom) {
                echo "HotelName: " . $room->name . '/ ' . $ar_name . " Done <br />";
            } else {
                echo $trRoom;
                echo "HotelName: " . $room->name . '/ ' .  $ar_name . " Failed <br />";
                break;
            }
        ...
        foreach ($hotels as $hotel) {
            try {
                ...
                if (!empty($hotel->rooms->room->roomType)) {
                    // print_r($hotel);
                    // exit;
                    if ($room->roomAmenities->amenity) var_dump($room->roomAmenities->amenity);
                    exit;

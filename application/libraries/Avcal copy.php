...
class Avcal
{
    ...
    public function gen_availability()
    {
        ...
        foreach ($this->AllPeriodsIntervals as $period) {
            if ($this->tdates('m', $period) == $this->target_month_num) {
                foreach ($this->_BaseSets() as $periodsBase) {
                    ...
                    $semptyRange = date_range($this->tdates('Y-m-01', $periodsBase->StartDate), $periodsBase->StartDate);
                    array_pop($semptyRange);
                    $Alldates = $this->filter_dates_key($periodsBase->SubDates);
                    $table_body .= "<tr><td>" . $periodsBase->R_Type_Sn . "</td>";
                    foreach ($baseRange as $base) {
                        $found = isset($Alldates[$base]);
                        if ($found) {
                            $table_body .= "<td>";
                            $table_body .= ($Alldates[$base] == 0) ? '<span style="color:red;padding:5px">X</span>' : '<span style="color:#098009;padding:15px">' . $Alldates[$base] . '</span>';
                            $table_body .= "</td>";
                        } else {
                            $table_body .= "<td style='background-color:#b53636;'> -E- </td>";
                        }

                    }
                    $table_body .= "</tr>";
                }
                $table_body .= '</tbody></table><hr>';
                echo $table_body;
            }
        }
    }
    ...
    public function gen()
    {
        ...
        foreach ($this->AllPeriodsIntervals as $period) {
            if ($this->tdates('m', $period) == $this->target_month_num) {
                ...
                $table_body = '';
                $baseRange = $this->month_range(); //month contain the base
                foreach ($this->_BaseSets() as $periodsBase) {

    ...
    <div class="container mt-3">
        ...
        <div id="inv" class="row">
            <div class="col">
                ...
                <button type="button" class="btn btn-danger font-weight-bold" data-toggle="modal"
                    data-target="#exampleModalCenter" @click="checkCancel">
                    <?= lang('resCancel') ?>
                </button>
                <!-- Modal -->
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <p v-if="!resDataLoading && Object.keys(cancellationDetails).length"> cancellation is
                                    allowed, charge is
                                    {{ cancellationDetails.charge +' '+cancellationDetails.currencyShort }}</p>
                                <p v-if="!resDataLoading && resDataError"> {{ resDataError }}</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-warning font-weight-bold"
                                    data-dismiss="modal"><?= lang('noThanks') ?></button>
                                ...
                                <button type="submit"
                                    class="btn btn-success font-weight-bold"><?= lang('yesSure') ?></button>
                                <?= form_close() ?>
                            </div>
                        </div>
                    </div>
        ...
        <?php endif;?>
            ...
            <div class="card">
                <div class="card-header p-3 invoice-head">
                    <p><span class="mcolor"><?= lang("invoicenumber") ?> </span><strong
                            class="text-white"><?php echo $idata->reservation_ref; ?></strong></p>
                    <?php if ($idata->Paid) : ?>
                    <span class="text-white"> <?= lang('status') ?> <span
                            class="text-success"><?= lang('paid') ?></span></span>
                    <?php else : ?>
                    <span class="text-white font-weight-bold"> <?= lang('status') ?> <span
                            class="warn"><?= lang('unpaid') ?></span></span>
                    <?php endif; ?>
                </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="row mb-4 mt-3">
                            ...
                            <div class="col-sm-6">
                                <img src="<?php echo  base_url('public_designs/assets/img/logo.png'); ?>"
                                    class="img-fluid" alt="Eomrah" width="300">
                            </div>
                       
                        <div class="col-sm-6 d-flex justify-content-end">
                            <div class="m-1">
                                <p class="text-dark font-weight-bold"><?= lang('bookingref') ?>: <span
                                        class="mcolor"><?= $idata->reservation_ref ?></span></p>
                                ...
                                <div class="text-center p-3">
                                    ...
                                    <img src="<?php echo  qrCode($idata->lat, $idata->lng); ?>" class="img-fluid"
                                        width="120" alt="Eomrah">
                    ...
                    <div class="row justify-content-between mb-4">
                        <div class="col-md-6 col-sm-12 pt-2">
                            ...
                            <span class="d-block"><strong
                                    class="font-weight-bold"><?= lang('hotelname') . '</strong>: ' . tolang($idata->Hotel_ID, 'hotelname') ?></span>
                            <span class="d-block"><strong
                                    class="font-weight-bold"><?= lang('hoteladdress') . '</strong>: ' . tolang($idata->Hotel_ID, 'hoteladdress') ?></span>
                        </div>
                        <div class="col-md-4 col-sm-12 pt-2 align-self-end">
                            <h5 class="font-weight-bold"> <?= lang('guestdetails') ?> </h5>
                            <span class="d-block"><strong class="font-weight-bold"><?= lang('guest_name') ?></strong>:
                                <?= $idata->guest_name ?? $idata->Public_UserFullName ?></span>
                            <span class="d-block"><strong class="font-weight-bold"><?= lang('phone_number') ?></strong>:
                                <?php echo $idata->Public_UserPhone; ?></span>
                            <span class="d-block"><strong class="font-weight-bold"><?= lang("checkin") ?></strong>:
                                <?php echo  $idata->CheckInDate; ?></span>
                            ...
                            <thead>
                                <tr style="background-color: #FFB300;">
                                    ...
                                    <th class="font-weight-bold">
                                        <?= (lang("additionals") == null) ? 'additionals' : lang("additionals") ?></th>
                    ...
                    <div class="row">
                        <div class="col-lg-4 col-sm-5 ml-auto text-center">
                            ...
                            <a class="btn pbtn p-3 font-weight-bold"
                                href="<?= site_url("payment/pay/$idata->reservation_ref/$mailHash") ?>">
                                <?= lang('paynow') ?></a>
                            <?php endif; ?>

                            <?php if ($idata->Paid):?>
                            <a class="btn pbtn p-3 font-weight-bold" href="<?=site_url("user/pdf/".$idata->reservation_ref)?>" target="__blank"><?=lang("download_pdf")?></a>
                            <?php endif;?>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="text-center">
                                <a class='btn-block btn-primary' href='<?php echo base_url('user/profile'); ?>'>Go
                                    Back</a>
                            </div>
        ...
        <div class="container mt-5 pb-5 error" style="content:'Not Found';">
            <div class="row m-5 p-5">
                <div class="col m-2 p-5 text-center">
                    <img src="<?= base_url('public_designs/assets/img/logo.png') ?>" class="img-fluid m-5" width="300px"
                        height="180px" alt="">
...
var inv = new Vue({
    ...
    methods: {
        checkCancel: async function() {
            this.resDataLoading = true;
            var params = new URLSearchParams();
            params.append('csrftkn', dataParams.csrftkn);
            params.append('resfref', '<?= $idata->reservation_ref ?>');
            params.append('resfrefd', <?= $idata->ID ?>);
            params.append('hmail', '<?= md5($idata->Public_UserEmail) ?>');
            await fetch(dataParams.surl + 'user/res_cancelCheck', {
                method: 'POST',
                body: params,
        ...
        book: function() {
            ...
            axios.post('<?= site_url('hotel/book') ?>', prms).then((res) => {
                if (res.data.status) {
                    window.location.href = '<?= site_url('reservation ') ?>';
                }
            }).catch((error) => {
                console.log(error);
            })

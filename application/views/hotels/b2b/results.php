            ...
            <div class="row justify-content-between">
                ...
                <div class="col-md-3 d-none d-lg-block pl-3">
                    <div class="px-3 sidebar">
                        <div class="row p-2">
                            <div class="col">
                            <label class="font-weight-bold"><?=lang('sea').' '.lang('hotel')?></label>
                             <input type="text" v-model="name_search" class="form-control"/>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <gmap-map :center="hotelInitialLatLng" :zoom="18" style="height: 600px; width: 100%;" ref="map"
                        ...
                        <gmap-marker :position="hotelInitialLatLng" :clickable="true"
                            ...
                            :title="thotel.name" ref="map" id="themap" map-type-id="roadmap"></gmap-marker>
...
Vue.component('search-result', {
    ...
    template: `<div :class="'row rounded p-md-4 mb-3 search_result' + (res.hotel.recommended?' border border-success':'')">
        ...
        <a :href="dataParams.surl +'b2b/hotel/details/'+ res.hotel.hslug +'/'+rslts.dt1+'/'+ rslts.dt2 +'/?rooms='+rslts.srooms+'&recrooms='+rslts.recrooms(res.hotel.rooms)"><img v-if="res.hotel.Main_Photo" class="img-fluid rounded" :src="res.hotel.Main_Photo"  :alt="res.hotel.Hotel_Name" style="height:230px; width:100%;"></a>
        </div>
        <div class="col res-sum">
        <div class="d-flex justify-content-between pt-2 mb-2">
            <a class="hotel-name":href="dataParams.surl +'b2b/hotel/details/'+res.hotel.hslug +'/'+rslts.dt1+'/'+rslts.dt2+'/?rooms='+rslts.srooms+'&recrooms='+rslts.recrooms(res.hotel.rooms)">
            ...
            <a :href="dataParams.surl +'b2b/hotel/details/'+res.hotel.hslug +'/'+rslts.dt1+'/'+rslts.dt2+'/?rooms='+rslts.srooms+'&recrooms='+rslts.recrooms(res.hotel.rooms)"
...
var rslts = new Vue({
    ...
    watch: {
        ...
        name_search:function(){
            this.resloading = true;
            setTimeout(this.filterByName, 200);
    ...
    methods: {
        ...
        pagereso: function() {
            var self = this;
            var url = dataParams.surl + '/b2b/hotel/pagereso';
        ...
        Searchhotels: async function() {
            var url = dataParams.surl + '/b2b/hotel/ajsearch?dest=' + this.destination + '&desttype=' + this
        ...
        filterByName: function() {
            var name = this.name_search;
            if (name.length) {
                this.sresults = this.sresults.filter(function(el) {
                 return el.hotel.Hotel_Name.toLowerCase().includes(name);
                });
            } else {
                this.sresults = this.csresults;
                this.filterByPrices();

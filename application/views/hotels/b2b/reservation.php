    ...
    <div class="row p-md-4 mt-md-3 mt-3">
        <div class="col-md-3 pb-3">
            <div class="res-summary">
                ...
                <div class="col p-3 ">
                    <p><strong class="mcolor font-weight-bold"> <?= !empty(lang('guests')) ? lang('guests') : 'Guests'; ?></strong> </p>
                    <p class="font-weight-bold"><?= $wallet['adults'] . lang('adults') . ',<br/> ' . ($wallet['childrens'] ? $wallet['childrens'] . ' Childs (Ages to be here as nyears, nyears)' : ''); ?></p>
                </div>
        ...
        <div class="col-md-9 checkout-form p-3">
            <div class="row">
                <div class="col-12 col-md-6">
                    <img src="<?= himg($hotel->Main_Photo) ?>" class="rounded img-fluid" width="400" height="200" alt="<?php echo $hotel->Hotel_Name; ?>" style="max-height:280px">
                </div>
                <div class="col-12 col-md-6">
                    <h5 class="hotel-name"><?php echo $hotel->Hotel_Name; ?></h5>
                    <h6 class="hotel-details-address mt-1"><i class="fas fa-map-marker-alt"></i> <?php echo $hotel->Hotel_Address; ?></h6>
                    <h6 class="hotel-details-phone mt-1"><i class="fa fa-phone-alt"></i> <?php echo $hotel->Hotel_Phone; ?></h6>
                </div>
            </div>
            <form class="form mt-3" method='post' action='<?php echo site_url('b2b/reservation/checkout'); ?>'>
                    ...
                    <div class="col">
                        ...
                        <div class="form-row mt-3">
                            <div class="form-group col-md-6 mt-4 mt-md-1">
                                <div class="p-label">
                                    <input class="form-control" id="FirstName" type="text" placeholder="<?= lang('first_name') ?>" v-model="user.fname">
                                    <label for="FirstName"><?= lang('first_name') ?><span class="text-danger">*</span></label>

                                </div>
                            </div>
                            <div class="form-group col-md-6 mt-4 mt-md-1">
                                <div class="p-label">
                                    <input type="text" class="form-control" id="FamilyName" placeholder="<?= lang('family_name') ?>" v-model="user.fmname">
                                    <label for="FamilyName"><?= lang('family_name') ?><span class="text-danger">*</span></label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-row mt-4">
                            <div class="form-group col-md-6 mt-4 mt-md-1 position-relative">
                                <div class="p-label">
                                    <input type="email" class="form-control" id="Email" placeholder="<?= lang('em') ?>" v-model="user.email" @focusout="validateEmail">
                                    <label for="Email">
                                        <?= lang('em') ?>
                            ...
                            <div class="form-group mt-4 mt-md-1 col-md-6">
                                <div class="p-label">
                                    <input type="email" class="form-control" id="EmailConfirm" placeholder="<?= lang('email_confirm') ?>" v-model="user.emailconfirm" @focusout="emailconfirm">
                                    <label for="EmailConfirm"><?= lang('email_confirm') ?><span v-if="emailmatch" style="color:red">
                            ...
                            <div v-if="emailexist" class="form-row tooltiped w-100">
                                <div class="form-group mt-4 mt-md-1 col">
                                    ...
                                    <span v-if="emailexist" class="d-block text-white font-weight-bold">{{ user.email }} <span class="mcolor"> <?= lang('alreadyexist') ?> </span></span>
                                    <p class="d-block py-2 font-weight-bold"><?= lang('confirm2ready') ?></p>
                                    <p v-if="!mailtokensent" class="text-center">
                                        <button type="button" class="btn ncbtn btn-sm" @click="confirmMailAcc"><?= lang('conf') ?></button>
                                        <a data-toggle="modal" href="#" ref="SignInModal" class="btn cbtn btn-sm" data-target="#signin"><?= lang('login') ?></a>
                                    </p>
                                    <div v-else class="form-row mt-1">
                                        ...
                                        <div class="form-group  mt-4 mt-md-1 col-md-4 col-sm">
                                            <button class="btn ncbtn bg-gcolor btn-sm" type="button" @click="confirmMailAccToken"><?= lang('verifyCode') ?></button>
                                        </div>
                                        <span><?= lang('nocode') ?> <a class="clink" @click="confirmMailAcc">, <?= lang('resend') ?></a></span>
                                    </div>
                                </div>
                            </div>
                            <!-- /html tooltip -->
                        ...
                        <div class="form-row mt-4">
                            ...
                            <div class="form-group mt-4 mt-md-1 col-md-6">
                                ...
                                <input type="tel" pattern="[0-9]{10}" class="form-control" id="phone" placeholder="<?= lang('phone_number') ?>" v-model="user.phone" @focusout="validatePhone">
                            </div>
                        </div>

                        <div class="form-group mt-4 mt-md-1">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="forwho" id="forme" value="0" v-model="forwho">
                                <label class="form-check-label" for="forme"><?= lang('self_reserve') ?></label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="forwho" id="forsomeoneelse" value="1" v-model="forwho">
                        ...
                        <div class="form-row" v-if="forwho == 1">
                            <div class="form-group mt-4 mt-md-1 col-md-6">
                                ...
                                <input type="text" class="form-control" id="GuestName" name="guestname" v-model="user.gname">
                            </div>
                            <div class="form-group mt-4 mt-md-1 col-md-6">
                                <label for="GuestEmail"><?= lang('guest_email') ?><span v-if="invalidgemail" style="color:red">
                                        {{invalidgemail}} </span></label>
                                <input type="email" name="guestemail" class="form-control" id="GuestEmail" v-model="user.gemail">
                            </div>
                    ...
                    <div class="col">
                        ...
                        <?php foreach ($wallet['items'] as $idx => $room) : if (!empty($room)) : ?>
                                <div class="bookedroom p-3">
                                    <h5 class="font-weight-bold"> <?= $room['rname'] . ' <small>(' . $room['meal'] . ')</small>'; ?></h5>
                                    <span>
                                        <span class="mcolor "> <?= str_replace('<br>', '', $room['cancelline']);   ?> <i class='fas fa-exclamation-circle' data-toggle='tooltip' data-placement='bottom' title='<?= $room['cancelref'] ?>' data-html='true' @click='ttip'></i></span>
                                    </span>
                                    <?php if (false) : ?>
                                    ...
                                    <div class="form-row mt-5">
                                        <div class="form-group mt-4 mt-md-1 col-md-6">
                                            <div class="p-label">
                                                <input id="gname<?= $idx + 1 ?>" class="form-control" type="text" name="guestname" placeholder="<?= lang('guest_name') ?>" v-model="rooms[<?= $idx ?>].gname" />
                                                <label for="gname<?= $idx + 1 ?>"><?= lang('guest_name') ?> <span class="text-danger">*</span></label>
                                            </div>
                                        </div>
                                        <div class="form-group mt-4 mt-md-1 col-md-6">
                                            <div class="p-label">
                                                <input id="gemail<?= $idx + 1 ?>" type="text" class="form-control" name="guestemail" placeholder="<?= lang('guest_email') . ' ' . lang('optional') ?> " v-model="rooms[<?= $idx ?>].gemail">
                                                <label id="gemail<?= $idx + 1 ?>"><?= lang('guest_email') ?><small class="text-success"> <?= lang('optional') ?> </small></label>
                                    ...
                                    <div v-if="rooms[<?= $idx ?>].allowsBeddingPreferencies" class="form-row">
                                        <div class="form-group mt-4 mt-md-1 col-md-6">
                                            ...
                                            <select id="roombedding<?= $idx + 1 ?>" class="form-control" v-model="rooms[<?= $idx ?>].bedpref">
                                    ...
                                    <div v-if="rooms[<?= $idx ?>].allowsSpecialRequests" class="form-row">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="spr<?= $idx + 1 ?>" v-model="rooms[<?= $idx ?>].spr" />
                                            <label class="form-check-label" for="spr<?= $idx + 1 ?>"><?= lang('SpReqs') ?></label>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group mt-4 mt-md-1 col">
                                            <div class="form-check">
                                                ...
                                                <?php foreach ($sp_requests as $spr) : ?>
                                                    <input class="form-check-input" type="checkbox" id="sp<?= $spr->specialreq_id . $idx ?>" value="<?= $spr->specialreq_code ?>" v-model="rooms[<?= $idx ?>].sp_requests">
                                                    <label class="form-check-label" for="sp<?= $spr->specialreq_id . $idx ?>"><?= $spr->specialreq ?></label>
                                                    <br />
                                                <?php endforeach; ?>
                                            </div>
                                        </div>
                ...
                <div class="row p-4">
                    <div class="col">
                        ...
                        ?>
                            ...
                            <button type="button" @click="bk" class="btn btn-block ebtn mt-2 p-2" :disabled="rsvd"><?= lang('check_out') ?></button>
                        <?php endif; ?>
                        <?php if (true) : ?>
                            <h4 class="text-center"><span><?= lang('p_methods') ?><span></h4>
                            <button type="button" @click="bk" class="btn btn-block ebtn mt-2 p-3 font-weight-bold" :disabled="rsvd"> <?= lang('check_out') ?></button>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
        ...
        methods: {
            bk: async function() {
                ...
                if (this.ulg) {
                    ...
                    params.append('rooms', JSON.stringify(this.rooms));
                    if (!this.user.fname.length || !this.user.fmname.length || !this.user.email.length || !this.user.emailconfirm.length ||
                    ...
                    if (this.forwho == 1) {
                        if (this.user.gname && this.user.gemail) {
                            ...
                            this.gdata = 'please complete Guest Details';
                            return;
                        }
                        params.append('pmethod', this.pmethod);
                    } else {
                ...
                await fetch(dataParams.surl + 'b2b/reservation/checkout', {
                    method: 'POST',
                    body: params,
                }).then((resp) => resp.json()).then((res) => {
                    console.log('checkout');
                    console.log(dataParams.surl + 'b2b/reservation/checkout');
                    console.log(res);
                    if (res.result) {
            ...
            uloggedin: async function(v) {
                await fetch(dataParams.surl + 'b2b/dashboard/check_login').then((resp) => resp.json()).then((res) => {
                    if (res.status) {
                        this.ulg = true;
                        this.rsv = false;
                    }
        ...
        computed: {
            rsvd: function() {
                if (this.ulg) {
                    ...
                    if (!this.user.fname.length || !this.user.fmname.length || !this.user.email.length || !this.user.emailconfirm.length || !this.user.phone.length || this.phoneerror || this.emailexist || this.phoneexist) {
